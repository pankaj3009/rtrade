...
createIndexConstituents <-
  function(redisdb, pattern, threshold = "2000-01-01") {
    ...
    for (i in 1:length(rediskeys)) {
      ...
      if (l > 0) {
        if (thresholdDate < as.Date(substring(rediskeys[i], nchar(pattern) + 2),
                                    format = "%Y%m%d",
                                    tz = "Asia/Kolkata")) {
    ...
    for (i in 1:length(rediskeysShortList)) {
      ...
      symbols <-
        data.frame(
          ...
          startdate = as.Date(seriesstartdate, format = "%Y%m%d", tz = "Asia/Kolkata"),
          enddate = as.Date(seriesenddate, origin = "1970-01-01"),
          stringsAsFactors = FALSE
        )
      for (j in 1:nrow(symbols)) {
        existingindex = which(dfstage2$symbol == symbols$symbol[j] &
...
createFNOConstituents <-
  function(redisdb, pattern, threshold = "2000-01-01") {
    ...
    for (i in 1:length(rediskeysShortList)) {
      ...
      seriesenddate = as.Date(substring(rediskeysShortList[i], nchar(pattern) +
                                          2),
                              format = "%Y%m%d",
                              tz = "Asia/Kolkata")
      symbols <-
        data.frame(
          symbol = names(unlist(redisHGetAll(
            rediskeysShortList[i]
          ))),
          startdate = as.Date(seriesstartdate, format = "%Y%m%d", tz = "Asia/Kolkata"),
    ...
    for (i in 1:length(rediskeysShortList)) {
      ...
      symbols <-
        data.frame(
          ...
          enddate = as.Date(seriesenddate, format = "%Y%m%d", tz = "Asia/Kolkata"),
  ...
  function(redisdb,
    ...
    for (i in 1:length(rediskeysShortList)) {
      ...
      if (exitdate >= periodstartdate &&
        ...
        netpercentprofit = percentprofit - (brokerage / (as.numeric(data["entryprice"]) *
...
GetCurrentPosition <-
  function(scrip,
    ...
    position <- 0
    if (nrow(portfolio) > 0) {
      portfolio<-portfolio[as.Date(portfolio$entrytime,tz="Asia/Kolkata")<=trades.till,]
      for (row in 1:nrow(portfolio)) {
        if ((is.na(portfolio[row, 'exittime']) || as.Date(portfolio[row, 'exittime'], tz = "Asia/Kolkata") > position.on) 
            &&  portfolio[row, 'symbol'] == scrip) {
          position = position + ifelse(grepl("BUY", portfolio[row, 'trade']),
                                       portfolio[row, 'size'], -portfolio[row, 'size'])
        }
      }
    }
    position
    ...
    if (nrow(portfolio) > 0) {
      ...
      for (l in 1:nrow(portfolio)) {
        ...
        if (length(entryindex) == 1) {
          ...
          dtindexend = which(pnl$bizdays == as.Date(md$date[exitindex], tz =
                                                      "Asia/Kolkata"))
          cumunrealized = seq(0, 0, length.out = (dtindexend -
                                                    dtindexstart + 1))
          ...
          for (index in entryindex:(exitindex - 1)) {
            ...
            #print(index)
            dtindex = which(pnl$bizdays == as.Date(md$date[index], tz =
                                                     "Asia/Kolkata"))
            if (length(dtindex) > 0) {
              # only proceed if bizdays has the specified md$date[index] value
              newprice = md$settle[index]
              pnl$unrealized[dtindex:nrow(pnl)] <-
                pnl$unrealized[dtindex:nrow(pnl)] + ifelse(grepl("BUY", side),
                                                           (newprice - mtm) * size,
                                                           (mtm - newprice) * size)
              cumunrealized[(dtindex -
                               dtindexstart + 1)] = ifelse(grepl("BUY", side),
                                                           (newprice - mtm) * size,
                                                           (mtm - newprice) * size)
              mtm <- newprice
              if (index == entryindex) {
        ...
        lastdaypnl = ifelse(grepl("BUY", side),
                            (portfolio[l, 'exitprice'] - mtm) * size,
                            (mtm - portfolio[l, 'exitprice']) * size)
        if (!unrealizedpnlexists) {
          if (length(dtindex) == 0) {
            dtindex = which(pnl$bizdays == as.Date(portfolio[l, "exittime"], tz = "Asia/Kolkata")) -
              1
          }
          pnl$realized[(dtindex + 1):nrow(pnl)] <-
            pnl$realized[(dtindex + 1):nrow(pnl)] + sum(cumunrealized) + lastdaypnl
          if (per.contract.brokerage) {
            pnl$brokerage[(dtindex + 1):nrow(pnl)] = pnl$brokerage[(dtindex + 1):nrow(pnl)] +
              brokerage[l] * size
          } else{
            pnl$brokerage[dtindex:nrow(pnl)] = pnl$brokerage[dtindex:nrow(pnl)] + brokerage[l] *
              size * portfolio[l, 'exitprice']
          }
          pnl$unrealized[(dtindex + 1):nrow(pnl)] = pnl$unrealized[(dtindex +
                                                                      1):nrow(pnl)] - sum(cumunrealized)
        } else{
          pnl$unrealized[(dtindex + 1):nrow(pnl)] = pnl$unrealized[(dtindex + 1):nrow(pnl)] +
            lastdaypnl
        }
    ...
    for (i in 1:nrow(signals)) {
      if (signals$inlongtrade[i] > 0) {
        ...
        if(length(indexofshort)>0){
                for (j in 1:length(indexofshort)) {
                        symbolsvector = unlist(strsplit(signals$symbol[indexofshort[j]], "_"))
                        expiry = as.Date(strptime(symbolsvector[3], "%Y%m%d", tz = "Asia/Kolkata"))
                        if (expiry == as.Date(signals$date[i], tz = "Asia/Kolkata")) {
                                signals$cover[i] = 1
                        }
    ...
    for (i in 1:nrow(signals)) {
      if (signals$sell[i] > 0) {
        ...
        if (length(indexofbuy) >= 1) {
          uniquesymbols = NULL
          for (j in 1:length(indexofbuy)) {
            if (length(grep(signals$symbol[indexofbuy[j]], uniquesymbols)) == 0) {
              ...
              if (nrow(datarow) == 1) {
                ...
                if (signals$sell[i] == 1) {
                  expiry = as.Date(strptime(symbolsvector[3], "%Y%m%d", tz = "Asia/Kolkata"))
                  if (as.Date(datarow$date, tz = "Asia/Kolkata") ==
                      expiry) {
                    if (symbolsvector[4] == "CALL") {
                      sellprice = max(0,
                                      udatarow$settle[1] - as.numeric(symbolsvector[5]))
                    } else{
                      sellprice = max(0,
                                      as.numeric(symbolsvector[5]) - udatarow$settle[1])
                    }
                  } else{
                  ...
                  expiry = as.Date(strptime(symbolsvector[3], "%Y%m%d", tz = "Asia/Kolkata"))
                  dte = businessDaysBetween("India",
                                            as.Date(signals$date[i], tz = "Asia/Kolkata"),
                                            expiry)
                ...
                if (signals$symbol[i] != signals$symbol[indexofbuy[j]]) {
                  ...
                  indexofsignal = which(out$date == signals$date[i] &
                ...
                if (as.Date(signals$date[i], tz = "Asia/Kolkata") ==
                    Sys.Date()) {
                  if (signals$symbol[i] != signals$symbol[indexofbuy[j]]) {
                    # add row to signals
                    out = rbind(
        ...
        if (length(indexofshort) >= 1) {
          uniquesymbols = NULL
          for (j in 1:length(indexofshort)) {
            if (length(grep(signals$symbol[indexofshort[j]], uniquesymbols)) == 0) {
              uniquesymbols[length(uniquesymbols) + 1] <-
            ...
            if (length(grep(signals$symbol[indexofbuy[j]], uniquesymbols)) == 0) {
              ...
              if (nrow(datarow) == 1) {
                ...
                if (datarow$open[1] != datarow$high[1] ||
                  ...
                  spread = (datarow$low[1] + datarow$settle[1] - (udatarow$low[1] + udatarow$settle[1])) /
                    2
                } else{
                  spread = (
                    datarow$high[1] + datarow$settle[1] - (udatarow$high[1] + udatarow$settle[1])
                  ) /
                    2
                }
                sellprice = NA_real_
      ...
      function(dates,
        ...
        for (i in 1:length(dates)) {
          ...
          for (s in 1:length(strikesUniverse)) {
            load(paste(fnodatafolder, expiry, "/", symbols[s], sep = ""))
